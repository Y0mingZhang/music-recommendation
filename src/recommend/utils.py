...
def convert_vector(b):
    ...
    """
    n = len(b)
    valid_idx = np.where(~np.isnan(b))[
        0
    ]  # 2d vector of nx1, only want row index of valid values
...
def strong_gen_preds(uk, X_test, sample_idx=[0]):
    ...
    for i in tqdm.tqdm(sample_idx):
        ...
        uk.gen_corrcoef(
            strong=True
        )  # one vector of test data, 3:22 (17,733 train vectors, 86 test vectors perturbations)
...
def timer(optimize_function):
    @functools.wraps(optimize_function)
    def f(self, *args, **kwargs):
        curr = time.time()
        retval = optimize_function(self, *args, **kwargs)
        print(f"optimization took {round(time.time() - curr)}s")
        return retval
